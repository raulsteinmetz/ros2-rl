...
import numpy as np
from model_based.mbpo.sac.networks import ActorNetwork, CriticNetwork, ValueNetwork
...
class Agent:
    ...
    def learn(self, memory): # problem is definetely here :(, with the state, state_, ... tensors being wrong
        ...
        state = T.tensor(state_batch, dtype=T.float).to(self.actor.device)
        state_ = T.tensor(next_state_batch, dtype=T.float).to(self.actor.device)
        action = T.tensor(action_batch, dtype=T.float).to(self.actor.device)
        reward = T.tensor(reward_batch, dtype=T.float).to(self.actor.device).unsqueeze(1)
        done = T.tensor(mask_batch).to(self.actor.device).unsqueeze(1)
