...
class SAC(object):
    def __init__(self, num_inputs, action_space, hyp):
        ...
        self.critic = QNetwork(num_inputs, action_space.shape[0], hyp.hidden_size).to(device=self.device)
        self.critic_optim = Adam(self.critic.parameters(), lr=hyp.lr)

        self.critic_target = QNetwork(num_inputs, action_space.shape[0], hyp.hidden_size).to(self.device)
        ...
        if self.policy_type == "Gaussian":
            ...
            self.policy = GaussianPolicy(num_inputs, action_space.shape[0], hyp.hidden_size, action_space).to(self.device)
            self.policy_optim = Adam(self.policy.parameters(), lr=hyp.lr)

        else:
            self.alpha = 0
            self.automatic_entropy_tuning = False
            self.policy = DeterministicPolicy(num_inputs, action_space.shape[0], hyp.hidden_size, action_space).to(self.device)
            self.policy_optim = Adam(self.policy.parameters(), lr=hyp.lr)
